...
<h2 align="center">
 🏥 HỆ THỐNG QUẢN LÝ HỒ SƠ BỆNH ÁN ĐIỆN TỬ PHI TẬP TRỤNG
...
Trong kỷ nguyên chuyển đổi số y khoa, việc số hóa hồ sơ bệnh án điện tử là một nhu cầu cấp thiết. Tuy nhiên, các kiến trúc phần mềm truyền thống thuộc thế hệ Web2 hầu hết đều vận hành dựa trên mô hình lưu trữ cơ sở dữ liệu tập trung (Centralized Database). Mô hình này bộc lộ những lỗ hổng bảo mật nghiêm trọng:
- **Nguy cơ can thiệp dữ liệu (Data Tampering):** Lịch sử chẩn đoán lâm sàng của bệnh nhân có thể bị chỉnh sửa lén lút bởi quản trị viên hệ thống hoặc tin tặc nhằm mục đích trục lợi bảo hiểm hoặc trốn tránh trách nhiệm pháp lý y khoa.
- **Điểm nghẽn hệ thống (Single Point of Failure - SPOF):** Khi máy chủ trung tâm của bệnh viện bị tấn công mã hóa tống tiền (Ransomware) hoặc gặp sự cố kỹ thuật, toàn bộ quy trình tra cứu hồ sơ bệnh lý bị đóng băng, gây nguy hiểm trực tiếp đến tính mạng bệnh nhân trong các tình huống cấp cứu.
- **Sự cố rò rỉ quyền riêng tư (Privacy Leaks):** Thông tin bệnh học nhạy cảm dễ bị khai thác và mua bán trái phép do thiếu cơ chế xác thực mã hóa mạnh mẽ phân quyền cá nhân.

### 1.2. Giải pháp ứng dụng "MedicalBlockchain"
Dự án mang tên **`MedicalBlockchain`** ra đời nhằm giải quyết triệt để các thách thức trên bằng cách ứng dụng nền tảng công nghệ mạng lưới chuỗi khối phi tập trung. Bằng cách di chuyển luồng dữ liệu cốt lõi lên Blockchain, hệ thống đạt được các tính chất ưu việt:
- **Tính bất biến tuyệt đối (Immutability):** Dữ liệu chẩn đoán một khi đã được đóng khối thì không một thế lực nào có thể chỉnh sửa hay xóa bỏ.
- **Định danh bằng mật mã học (Web3 Identity):** Y bác sĩ tương tác với hệ thống hoàn toàn thông qua cặp khóa công khai/khóa bí mật của ví điện tử cá nhân (MetaMask), loại bỏ cơ chế tài khoản-mật khẩu truyền thống dễ bị đánh cắp.
- **Kiến trúc đồng bộ lai (Hybrid Architecture):** Hệ thống tích hợp phân hệ Mock Storage nội bộ kết hợp thuật toán băm Keccak-256, vừa bắt MetaMask ký số thực hiện giao dịch On-chain thật, vừa đồng bộ bộ nhớ đệm tại Client nhằm đảm bảo trải nghiệm kết xuất dữ liệu mượt mà, phản hồi tức thời dưới vài mili-giây.
...
- **Solidity (v0.8.28):** Ngôn ngữ lập trình hướng đối tượng kiểu tĩnh dùng để biên soạn hợp đồng thông minh y tế lõi.
...
- **Ethers.js (v5.7.2):** Thư viện JavaScript cầu nối giúp khởi tạo các thực thể `Web3Provider` và kết nối trực tiếp đến mã nhị phân giao diện ứng dụng ABI của Smart Contract.
...
1	Kiểm tra hệ thống khi trình duyệt chưa thiết lập tích hợp ví.	Truy cập dApp bằng trình duyệt ẩn danh không cài MetaMask.	Hệ thống nhận diện, hiển thị thông báo yêu cầu cài đặt và khóa các nút chức năng.	Hiển thị đúng chuỗi cảnh báo, vô hiệu hóa toàn bộ form nhập liệu.	ĐẠT
